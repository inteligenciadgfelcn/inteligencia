...
import { DragEndEvent, Map as LeafletMap, icon } from 'leaflet'
...
type Punto = [string, string, string?]

interface MapaStoryArgs {
  puntos: Punto[]
}
...
export default {
  ...
  parameters: {
    docs: {
      ...
      description: {
        component:
          'Componente que utiliza la biblioteca Leaflet para mostrar un mapa interactivo en una aplicación web.',
      },
    },
  },
} as Meta

const BaseTemplate: StoryFn<MapaStoryArgs> = ({ puntos }) => {
  const [zoom, setZoom] = useState<number>()
  const [centro, setCentro] = useState<[number, number] | undefined>()
  const [puntosState, setPuntosState] = useState<Punto[]>([])

  const mapRef = useRef<LeafletMap>(null)

  const agregarPunto = (latlng: [number, number]) => {
    setPuntosState([[latlng[0].toString(), latlng[1].toString()]])
  }

  const dragEvent = (e: DragEndEvent) => {
  ...
  const clickMarker = (index: number) => {
    const marker = puntosState[index]
  ...
  useEffect(() => {
    if (puntos) {
      ...
      setZoom(calcularZoom(puntos.map((p) => [Number(p[0]), Number(p[1])])))
    }
  }, [puntos])

  useEffect(() => {
    const nuevoCentro = getCentro(
      puntosState.map((p) => [Number(p[0]), Number(p[1])])
    )
    setCentro(nuevoCentro ? [nuevoCentro[0], nuevoCentro[1]] : undefined)
  }, [puntosState])
    ...
    puntosState
      ...
      .map((punto, index) => (
        <Marker
          key={`${index}-marker`}
          icon={ICON}
          draggable={puntosState.length < 2}
          eventHandlers={{
            dragend: dragEvent,
  ...
  return (
    <Mapa
      ...
      zoom={zoom}
      centro={centro}
      onClick={puntosState.length < 2 ? agregarPunto : undefined}
      markers={<Markers />}
    />
  )
}

export const PorDefecto = BaseTemplate.bind({})
PorDefecto.args = { puntos: [] }

const puntosExample: Punto[] = [
  ['-16.5000', '-68.1500'],
  ['-17.7833', '-63.1833'],
  ['-17.3895', '-66.1568'],
  ['-19.0333', '-65.2627'],
...
export const SoloLectura = BaseTemplate.bind({})
SoloLectura.args = { puntos: puntosExample }

export const SoloLecturaConTooltip = BaseTemplate.bind({})
SoloLecturaConTooltip.args = { puntos: puntosExample }

const puntosTuristicosBolivia: Array<Punto> = [
  //La Paz
...
export const LugaresTuristicosBolivia = BaseTemplate.bind({})
LugaresTuristicosBolivia.args = { puntos: puntosTuristicosBolivia }
