...
export function DatosGeneralesForm({
  ...
  useEffect(() => {
    ...
    const cargarItemsOperativo = async () => {
      ...
      try {
        const respuesta =
          await SiiiLookupsService.obtenerItemsOperativo(
            idCategoria
          )
        if (!activo || !respuesta?.finalizado) return

        const opciones = respuesta.datos.map((t: { id: any; descripcion: any }) => ({
          id: String(t.id),
          value: String(t.id),
          label: t.descripcion,
        }))
        ...
        const existeItemActual = opciones.some(
          (opcion: { value: any }) => Number(opcion.value) === idItemActual
        )
    ...
    try {
      ...
      if (idCaso > 0) {
        ...
        if (tieneOperativo) {
          ...
          if (idOperativo > 0) {
            await DatosGeneralesService.actualizarOperativo(
              idOperativo,
              payloadOperativo
            )
            Alerta({
          ...
          await DatosGeneralesService.crearOperativo(
            idCaso,
            payloadOperativo
          )
          Alerta({
        ...
        idCategoria > 0
          ? (async () => {
            try {
              ...
              if (res?.finalizado) {
                setOpcionesOperativoEn(
                  res.datos.map((t: { id: any; descripcion: any }) => ({
                    id: String(t.id),
                    value: String(t.id),
                    label: t.descripcion,
                  }))
