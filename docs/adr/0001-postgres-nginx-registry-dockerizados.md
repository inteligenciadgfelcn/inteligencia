...
- **Qué copiar**: las carpetas `docs/templates/postgres/`, `docs/templates/nginx/`, `docs/templates/registry/` y el archivo `docs/templates/docker-compose.prod.yml`, tal cual — ya están probadas, no hace falta reescribir nada desde cero.
- **Si algo de la doc no calza** con lo que realmente hace falta ejecutar en el servidor real (un comando que falla, una ruta que no existe, una variable que no aplica): es un bug de la documentación, no una señal para improvisar por fuera de ella. Corregirlo ahí mismo (el archivo `.md` o la plantilla bajo `docs/templates/`) para que quede capturado, con una nota de qué se probó y qué falló — mismo criterio que se usó para armar este ADR.
