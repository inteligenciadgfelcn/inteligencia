...
bash deploy/tools/registry/build-and-push.sh [tag]
```
...
Sin argumento, `tag` es el short SHA del commit actual (`git rev-parse --short HEAD`) — así cada imagen queda trazable al commit exacto que la generó. El script buildea y pushea las 3 imágenes (`felcn-auth-backend`, `felcn-base-backend`, `felcn-base-frontend`) parado en la raíz del repo.
