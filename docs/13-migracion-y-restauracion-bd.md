...
No existe Liquibase, Flyway ni ninguna herramienta de migración externa en este proyecto. Cada backend NestJS (`felcn-auth-backend`, `felcn-base-backend-v2`) trae su propio esquema TypeORM, con dos conjuntos de archivos versionados en `database/migrations/` (estructura: tablas, columnas, índices) y `database/seeds/` (datos: catálogos, políticas Casbin, módulos de menú, usuario admin inicial):
...
|---|---|---|
| **Desarrollo local / dev (`servertest`)** | Dump/backup restaurado desde otro ambiente (`.sql`/`.dump` vía `pg_restore`/`psql`) **o** `npm run setup` desde cero — indistinto, es un ambiente de trabajo | No hay compromiso de integridad de datos; el objetivo es tener algo con qué probar |
| **Staging (servidor nuevo, 21/08/2026)** | Dump ya generado el 21/08/2026 de las 8 bases reales (`backups/20260821-staging-migracion/`, fuera de git) — ver [07-servidor-nuevo-desde-cero.md](./07-servidor-nuevo-desde-cero.md) §3 | Staging necesita parecerse a producción/dev en volumen de datos para pruebas realistas, así que sí se restaura un dump con datos |
