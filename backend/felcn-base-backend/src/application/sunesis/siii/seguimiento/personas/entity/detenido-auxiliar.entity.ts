...
/**
 * Entidad DetenidoAuxiliar
 * Personas implicadas en un operativo (módulo jurídico-seguimiento).
 * Origen: SQL Provided — public.detenido_auxiliar
 */
...
export class DetenidoAuxiliar {
  ...
  insertarFechaIngreso() {
    ...
    if (!this.fechaHoraActualizacion) {
      this.fechaHoraActualizacion = new Date()
    }
  }
