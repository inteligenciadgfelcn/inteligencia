...
export class SegipService extends BaseService {
  ...
  async contrastar(datosPersona: PersonaDto, retornarPrimerError = true) {
    try {
      ...
      const tipoDocumento = datosPersona.tipoDocumento ?? TipoDocumento.CI

      const tipoPersonaSegip =
        tipoDocumento === TipoDocumento.CI
          ? TipoPersonaSegip.NACIONAL
          : TipoPersonaSegip.EXTRANJERO

      const respuesta = await firstValueFrom(
        this.httpService
          .get(urlContrastacion, {
            params: {
              tipo_persona: tipoPersonaSegip,
