...
export class AuditoriaSesionInterceptor implements NestInterceptor {
  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
    ...
    const user = request.user as PassportUser | undefined

    if (!user?.id || !/^\d+$/.test(String(user.id))) return next.handle()

    const userId = String(user.id)
    const userName = String(user.usuario ?? user.id)
      .replace(/'/g, '')
      .substring(0, 50)
    ...
    Promise.all(queries.map((q) => this.dataSource.query(q).catch(() => undefined)))
